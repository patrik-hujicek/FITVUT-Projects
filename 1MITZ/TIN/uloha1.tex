...
1. $N^{'} = N \cup  \{S^{'}\}, S^{'} \notin N$\\
2. $\Sigma^{'} =  \Sigma$ \\
% \noindent 3. Množinu pravidiel $P^{'}$ definujeme takto:
%\begin{itemize}
%	\item $\forall (A \rightarrow xB) \in P \land A, B \in N, x \in \Sigma^{*}: \exists (B %\rightarrow Ax) \in P^{'}$
%	\item $\forall (A \rightarrow x) \in P \land A \in N, x \in \Sigma^{*}: \exists(S^{'} %\rightarrow Ax) \in P^{'}$
%	\item $\exists (S \rightarrow \varepsilon) \in P^{'}$
%\end{itemize}
3. $P^{'} =$
\begin{gather*}
